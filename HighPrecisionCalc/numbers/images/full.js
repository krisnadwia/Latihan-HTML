...
function fullMain(imode) {
    this.version = 'V1.0';
    this.w = 480;
    this.h = 300;
    this.mode = typeof imode !== 'undefined' ? imode : 'Multiply';
    my.nDec = 200
    Decimal.set({ precision: 1000, toExpNeg: -200, toExpPos: 200 })
    var id = "calc";
    this.keys = ["7", "8", "9", "\u00d7", "\u00f7", "4", "5", "6", "+", "\u2212", "1", "2", "3", "(", ")", "0", ".", "^", "\u2190", "C"];
    this.keysn = this.keys.length;
    this.fkeys = ["!", "e", "pi", "&nbsp;"];
    var s = '';
    s += '<div class="js" style="position:relative; max-width:480px; min-height:' + h + 'px; border: none; border-radius: 20px; margin:auto; display:block;">';
    s += '<textarea id="u" class="input" style="width:98%; height:60px; font: 25px Arial; text-align:center; z-index:2; border-radius: 10px;" onkeyup="userKey(event)" autofocus >&nbsp;</textarea>';
    s += '<div style="display: block; position:relative; z-index:5;">';
    s += '<div id="a" class="output" style="display: inline-block; margin: 0px 0 5px 0; width:98%; padding:3px; font: bold 18px Arial; text-align: center; word-wrap: break-word;" >&nbsp;</div>';
    s += '<button id="rad" class="btn" style="position: absolute; right:0px; top: 28px; font-size: 14px; visibility:hidden; " onclick="toggleRad()" >rad</button>';
    s += '</div>';
    s += getKeyHTML();
    s += '<br>';
    s += getfKeyHTML();
    s += '<br>';
    s += '<div style="font: bold 12px arial; text-align:left;">History:</div>';
    s += '<textarea id="hist" class="output" value="ddd" style="width:98%; height: 120px; font: 14px Arial; display: block; text-align: center;">';
    s += '</textarea>';
    s += '<div style="font: bold 12px arial; margin: 3px;  text-align:center;">';
    s += 'Decimals: <input id="decN" class="input" value="' + my.nDec + '" style="text-align: center; width:60px;" onkeyup="decNChg()"></input>';
    s += '</div>';
    s += '<div style="font: 11px arial; color: #6600cc; text-align:center;">' + this.version + '</div>';
    s += '</div>';
    document.write(s);
    hists = [];
    lastHist = "?";
...
function tests() {
    exs = ['5!', '3-1.1e3/1e2 -45', '1E4', '3-1e-4', '-2.2e-4-3', '3*sqrt(2)', 'pi/2', '2pi', 'e^2', '2e', '27', 'asin(1)', '$3,600.5/2', '1+2-3', '3(7+3)', '1/sqrt(2)', 'sqrt(-1)', '12+2*3', 'abs(-3.2)', 'floor(-3.2)', 'ceil(-3.2)', 'sign(-3.2)', 'round(-3.2)', 'sinh(-1.1)', '2.e2', '2^0.5', '2^-0.5', '2^-.5', '3.1*-3', '3.1*-3.1'];
    exNo = 0;
    for (var i = 0; i < exs.length; i++) {
        var ex = exs[i];
        p.newParse(ex);
        var s = p.getVal().toPrecision(my.nDec);
        var num = fmtNum(s);
        console.log("", ex, '==>', num);
    }
}
...
function toggleRad() {
    ...
    if (this.radsQ) { inddiv.innerHTML = 'rad'; } else { inddiv.innerHTML = 'deg'; }
    doCalc(false);
    return;
}
...
function doKey(i) {
    var key = ''
    if (i < this.keysn) { key = this.keys[i]; } else { key = this.fkeys[i - this.keysn]; }
    var simples = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "(", ")", "+", "\u00d7", "\u00f7", "\u2212", ".", '^', 'i'];
    if (simples.indexOf(key) >= 0) { pushU(key); } else {
        switch (key) {
            case "=":
                var x = doCalc(true);
                break;
...
function aroundU(s1, s2) {
    ...
    var ops = ["+", "-", "\u2212", '*', "\u00d7", '/', "\u00f7", '^'];
    var skipi = false;
    for (var i = s.length - 1; i >= 0; i--) { var c = s.charAt(i); if (c == 'i') skipi = true; if (skipi && (c == '-' || c == '+')) { skipi = false; } else { if (ops.indexOf(c) > -1) break; } }
    var sNew = ''
    if (i == 0) { sNew = s1 + s + s2; } else {
        i++;
...
function userKey(e) {
    ...
    switch (kt) {
        ...
        case '^':
            ...
            if (f.length == 1) { my.uDiv.value = hists[0][0] + f; }
            doCalc(false);
            break;
        default:
            doCalc(false);
...
function decNChg() {
    my.nDec = parseInt(document.getElementById('decN').value)
    if (isNaN(my.nDec)) { my.nDec = 1 }
    if (my.nDec < 1) {
        my.nDec = 1
        document.getElementById('decN').value = my.nDec
    }
    if (my.nDec > 10000) {
        my.nDec = 10000
        document.getElementById('decN').value = my.nDec
    }
    console.log('my.nDec', my.nDec)
    Decimal.set({ precision: my.nDec + 10, toExpNeg: -200, toExpPos: 200 })
    doCalc(false)
}
...
function doCalc(updateUQ) {
    ...
    this.p.newParse(f);
    var s = this.p.getVal().toPrecision(my.nDec);
    var num = fmtNum(s);
    console.log("doCalc," + f + ', ' + this.p.errMsg + ', ' + p.rootNode.walkFmt() + ', ' + num);
    if (this.p.errMsg.length > 0) { document.getElementById('a').innerHTML = '&nbsp;'; } else {
        if (updateUQ) { my.uDiv.value = ''; }
...
function addHist(q, a) {
    if (q != a.toString()) {
        var hist = q + " = " + a;
        if (hist != lastHist) {
            if (lastHistTime + 2000 > Date.now()) { hists[0] = [q, a]; } else { hists.unshift([q, a]); }
            lastHist = hist;
            lastHistTime = Date.now();
            var h = '';
            for (var i = 0; i < Math.min(8, hists.length - 1); i++) { h += hists[i][0] + ' = ' + hists[i][1] + "\n"; }
...
function getHist() {
    ...
    lastHistTime = 0;
    update();
    for (var i = 0; i < hists.length; i++) { s += hists[i] + "\n"; }
    return s;
}
...
function Parser() {
    ...
    this.radiansQ = true;
    this.vals = [];
    for (var i = 0; i < 26; i++) { this.vals[i] = 0; }
    this.reset();
}
Parser.prototype.setVarVal = function(varName, newVal) {
    switch (varName) {
        ...
        case "z":
            ...
            break;
        default:
            if (varName.length == 1) { this.vals[varName.charCodeAt(0) - 'a'.charCodeAt(0)] = newVal; }
    }
};
Parser.prototype.getVal = function() { return (this.rootNode.walk(this.vals)); };
...
Parser.prototype.fixPercent = function(s) {
    if (!s.match(/%/)) { return s; }
    var myRe = /[0-9]*\.?[0-9]+[%]/g;
    var bits = [];
    var stt = 0;
...
Parser.prototype.fixENotation = function(s) {
    if (!s.match(/e/i)) { return s; }
...
Parser.prototype.fixParentheses = function(s) {
    var sttParCount = 0;
    var endParCount = 0;
    for (var i = 0; i < s.length; i++) { if (s.charAt(i) == "(") sttParCount++; if (s.charAt(i) == ")") endParCount++; }
...
Parser.prototype.parse = function(s) {
    ...
    if (bracStt > -1) {
        ...
        var isParam = false;
        if (bracStt == 0) { isParam = false; } else {
        ...
        if (!isParam) { this.tempNode.push(this.parse(s.substr(bracStt + 1, bracEnd - bracStt - 1))); return this.parse(s.substr(0, bracStt) + "$" + (this.tempNode.length - 1).toString() + s.substr(bracEnd + 1, s.length - bracEnd - 1)); } else {
            var startM = -1;
            for (var u = bracStt - 1; u > -1; u--) { var found = this.operators.indexOf(s.substr(u, 1)); if (found > -1) { startM = u; break; } }
...
MathNode.prototype.getLevelsHigh = function() {
    var lvl = 0;
    for (var i = 0; i < this.childCount; i++) { lvl = Math.max(lvl, this.child[i].getLevelsHigh()); }
    return (lvl + 1);
};
MathNode.prototype.isLeaf = function() { return (this.childCount == 0); };
...
MathNode.prototype.walkFmta = function(noparq, prevop) {
    ...
    if (this.childCount > 0) {
        ...
        for (var i = 0; i < this.childCount; i++) {
            ...
            if (this.typ == this.tFUNC || (parq && i > 0)) { s += ")"; }
        }
    } else { s += this.fmt(); if (prevop == "sin" || prevop == "cos" || prevop == "tan") { if (this.radiansQ) { s += " rad"; } else { s += "\u00b0"; } } }
    return s;
};
...
MathNode.prototype.walk = function(vals) {
    ...
    if (this.typ == this.tVAR) {
        switch (this.v) {
            ...
            case "z":
                return (vals[25]);
            case "pi":
                return new Decimal('3.');
            case "e":
                return new Decimal('2.');
            case "a":
                return (vals[0]);
            case "n":
                return (vals[13]);
    ...
    if (this.typ == this.tOP) {
        var val;
        for (var i = 0; i < this.childCount; i++) {
            var val2 = new Decimal('0');
            if (this.child[i] != null)
                val2 = this.child[i].walk(vals);
            if (i == 0) { val = val2; } else {
                switch (this.op) {
                    case "add":
                        val = val.add(val2);
                        break;
