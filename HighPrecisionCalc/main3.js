...
function absolute(base, relative) {
    ...
    stack.pop();
    for (var i = 0; i < parts.length; i++) { if (parts[i] == ".") continue; if (parts[i] == "..") stack.pop();
...
function doSpell() { if (typeof(reSpell) == 'undefined') return; var userLang = window.navigator.userLanguage || window.navigator.language; switch (userLang.toLowerCase()) {
        case "en-us":
            break;
        case "en-au":
        case "en-ca":
        ...
        case "en-nz":
        case "en-za":
            fixSpells(document.body, "s"); break;
        default:
    } }
...
function doChina() {
    ...
    if (zhQ) {
        ...
        if (transQ) { path = 'http://www.shuxuele.com' + path; var newDiv = document.createElement("div"); var tgtDiv = document.getElementById("content");
            document.body.insertBefore(newDiv, tgtDiv); var str = '<div style="position: absolute; right: 50%; margin-right: 260px; top:120px; font:45px Arial; background-color: rgba(255, 220, 0, 0.5);  border-radius:5px; border: 1px solid red; "><a href="' + path + '" style="color:red; text-decoration: none;">ä¸­æ–‡</a></div>';
            newDiv.innerHTML = str; }
    }
    if (!transQ) { var path = location.pathname;
        path = path.split('/mathsisfun/').join('/'); var dir = path.substring(0, path.lastIndexOf('/')); var file = path.substring(path.lastIndexOf('/') + 1); var fbQ = false; if (fbQ) { path = 'https://www.facebook.com/Mathisfun-249324325634773/'; var newDiv = document.createElement("div"); var tgtDiv = document.getElementById("content");
            document.body.insertBefore(newDiv, tgtDiv); var str = '<div style="position: absolute; right: 50%; margin-right: 260px; top:120px; "><a href="https://www.facebook.com/Mathisfun-249324325634773/" rel="nofollow"><img src="/images/style/facebook.svg"></a></div>';
            newDiv.innerHTML = str; } }
}

function fixSpells(elem, tp) { if (!(elem instanceof Node) || elem.nodeType !== Node.ELEMENT_NODE) return; var children = elem.childNodes; for (var i = 0; children[i]; ++i) { var node = children[i]; switch (node.nodeType) {
            case Node.ELEMENT_NODE:
                fixSpells(node, tp); break;
            case Node.TEXT_NODE:
                if (tp == "s") fixSpell(node); if (tp == "d") fixDec(node); break; } } }
...
function fixSpell(node) {
    ...
    if (s.match(/(?=.*[a-zA-Z])/)) {
        var sStt = s;
        for (var j = 0; j < reSpell.length; j++) { var s0 = reSpell[j][0]; var s1 = reSpell[j][1];
...
function fixDec(node) { var s = node.nodeValue; var sStt = s;
    s = s.replace(/(\d),(\d\d)/g, '$1#$2').replace(/(\d)\.(\d)/g, '$1,$2').replace(/(\d)#(\d)/g, '$1.$2'); if (s != sStt) { node.nodeValue = s; } }
...
function openEnglish() { if (typeof tranfrom == 'undefined') tranfrom = 'index.htm'; var path = tranfrom; var url = "https://www.mathsisfun.com/" + path;
    window.location = url; }
...
function Citation() { var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; var path = location.pathname + location.search; var atitle = document.title; var md = new Date(document.lastModified); var mDate = md.getDate() + " " + months[md.getMonth()] + " " + md.getFullYear(); if (typeof Author == 'undefined') Author = 'Pierce, Rod';
    postWith("https://" + document.domain + "/citation.php", { path: path, title: atitle, moddate: mDate, author: Author }); }

function Contribute() { var path = location.pathname + location.search; var atitle = document.title;
    postWith("https://" + document.domain + "/contribute.php", { path: path, title: atitle }); }
...
function URLEncode(text) {
    ...
    var s = "";
    for (var i = 0; i < text.length; i++) { var ch = text.charAt(i); if (ch == " ") { s += "+"; } else if (SAFECHARS.indexOf(ch) != -1) { s += ch; } else { var charCode = ch.charCodeAt(0); if (charCode > 255) { s += "+"; } else { s += "%";
...
function CopyToClipboard(txtArea) { txtArea.focus();
    txtArea.select(); var copiedTxt = document.selection.createRange();
...
function putFlash6(w, h, fn, querystring, clr, noflash) {
    ...
    if (hasFlash()) {} else {
        var s = "";
        if (noflash) { s = noflash; } else { s = '<a href="/flash-player.html"><img src="/images/style/no-flash.jpg" alt="Needs Flash Player"></a>'; }
        document.write(s);
        if (Math.random() < 0.1) { var path = location.pathname; var pg = path + ", " + fn; var pgHex = toHex(pg);
...
function toHex(s) {
    var hex = '';
    for (var i = 0; i < s.length; i++) { var cc = s.charCodeAt(i).toString(16); if (cc.length < 2) cc = '0' + cc;
        hex += '' + cc; }
    return hex;
}

function hasFlash2() {
    try { var fo = new ActiveXObject('ShockwaveFlash.ShockwaveFlash'); if (fo) return true; } catch (e) { if (navigator.mimeTypes && navigator.mimeTypes["application/x-shockwave-flash"] && navigator.mimeTypes["application/x-shockwave-flash"].enabledPlugin) return true; }
    return false;
}

function hasFlash() {
    var isPPAPI = false;
    try { var fo = new ActiveXObject('ShockwaveFlash.ShockwaveFlash'); if (fo) return true; } catch (e) {
        var type = 'application/x-shockwave-flash';
        var mimeTypes = navigator.mimeTypes;
        var endsWith = function(str, suffix) { return str.indexOf(suffix, str.length - suffix.length) !== -1; }
        if (mimeTypes && mimeTypes[type] && mimeTypes[type].enabledPlugin && (mimeTypes[type].enabledPlugin.filename == "pepflashplayer.dll" || mimeTypes[type].enabledPlugin.filename == "libpepflashplayer.so" || endsWith(mimeTypes[type].enabledPlugin.filename, "Chrome.plugin"))) isPPAPI = true;
        console.log("PPAPI enabled: " + isPPAPI);
    }
    return isPPAPI;
}

function FlashPHP(w, h, pathtoswf, clr) { var path = location.pathname;
    path = URLEncode(path.substring(0, path.lastIndexOf('/'))); var atitle = URLEncode(document.title); var url = "https://" + document.domain + "/flash.php?path=" + path + "/" + pathtoswf + "&w=" + w + "&h=" + h + "&col=" + URLEncode(clr) + "&title=" + atitle;
    window.location = url; }

function sethideadscookie(val) {
    var date = new Date();
    if (val == 1)
        date.setTime(date.getTime() + (1 * 36 * 60 * 60 * 1000));
    else
...
function printImg(s) { var pwin = window.open(s, "_blank");
...
function getQ() {
    var qs = "";
    for (var i = 0; i < arguments.length; i++) { if (i > 0) qs += "_";
        qs += arguments[i]; }
    var s = "";
    for (i = 0; i < arguments.length; i++) { s += '<a href="javascript:doQ(' + arguments[i] + ",'" + qs + "'" + ')">Question&nbsp;' + (i + 1) + '</a> '; }
    document.write(s);
}
...
function showMenu() {
    ...
    if (menuVis) { div.style.display = 'none'; } else { div.style.display = 'block'; if (srchVis) showSrch(); }
    menuVis = !menuVis;
}
...
function showSrch() {
    ...
    if (srchVis) { div.style.display = 'none'; } else { div.style.display = 'block'; if (menuVis) showMenu(); }
    srchVis = !srchVis;
}

function endsWith(str, suffix) { return str.indexOf(suffix, str.length - suffix.length) !== -1; }

function getSearch() { var s = "";
...
function getAdRight2() {
    ...
    if (true) {
        ...
        s += '}';
        s += '</script>';
        if (true) { s += '	<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit"></script>'; } else {
...
function getTrans() {
    ...
    s += '}';
    if (false) { s += '</script>';
        s += '	<script type="text/javascript" src="//translate.google.com/translate_a/element.js?cb=googleTranslateElementInit"></script>'; } else {
...
function getSocial() { if (true) return ''; var url = location.href; var title = encodeURIComponent(document.title); var s = '';
...
function getLinks() { var url = location.href; var urlenc = encodeURIComponent(location.href); var title = encodeURIComponent(document.title); var linkstt = '<a target="_blank" rel="nofollow" href="'; var s = '';
    ...
    s += '<a href="javascript:tellAFriend()" id="linkem" title="eMail">eMail a Friend</a>'; return s; }
    ...
    switch (theme) {
        ...
        case 'dark':
            props = [
                ...
                ['--a0Clr', 'hsla(50, 100%, 96%, 50%)'],
                ['--a1Clr', 'hsl(0, 0%, 9%)'],
                ['--a2Clr', 'hsl(45, 100%, 27%)'],
                ['--a3Clr', 'hsl(50, 100%, 85%)'],
                ['--a4Clr', 'hsl(50, 100%, 96%)'],
                ['--b0Clr', 'hsla(214, 100%, 50%, 15%)'],
                ['--b1Clr', 'hsl(214, 100%, 20%)'],
                ['--b2Clr', 'hsl(214, 65%, 46%)'],
...
function logVisit() { var pg = location.pathname; if (pg == '/') return; var pgHex = toHex(pg);
    addVisit(pgHex, "visit", window.location.hostname); }
if (Math.random() < 0.1) logVisit();

function addVisit(pg, viewtype, hostname) { console.log("addVisit", viewtype, hostname); var req;
    req = new XMLHttpRequest(); var params = "type=" + viewtype;
    params += "&site=mif";
...
function onScroll() { var scrollTop = document.documentElement ? document.documentElement.scrollTop : document.body.scrollTop; var menu = document.getElementById("menu"); if (scrollTop > 100) { menu.style.top = "0px";
...
function getRelated() {
    ...
    var right = null;
    var left = null;
    if (count > 0) { right = links[0]; if (count > 1) { left = links[count - 1]; } }
    var pgWd = 760;
    var linkWd = 110;
    var wd = ((window.innerWidth - pgWd) / 2 - linkWd) / 2 + pgWd / 2;
    wd = Math.max(380, wd);
    var s = '';
    if (left != null) { s += '<div style="position: fixed; top: 0px; right: 50%; margin: 10px ' + wd + 'px 10px 0; text-align: left; ">';
        s += fmtMenuBox(left.href, left.text, 0);
        s += '</div>'; }
    if (right != null) { s += '<div style="position: fixed; top: 0px; left: 50%; margin: 10px 0 10px ' + wd + 'px; text-align: right; ">';
...
function doVideo(id, spanid) {
    ...
    if (visQ) { var frame = document.getElementById(spanid + "v1");
        frame.parentNode.removeChild(frame);
        videoVis.splice(visIndex, 1);
        visQ = false; } else { s += '<div class="center">';
...
function resizeVideo(spanid) { var v1 = document.getElementById(spanid + "v1"); var wd = window.innerWidth - 40; if (wd > 640) wd = 640;
